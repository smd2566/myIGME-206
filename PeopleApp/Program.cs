...
namespace PeopleApp
{
    ...
    //Purpose: To allow the user to enter a series of courses and personal info and to be displayed back
    //Restrictions: Adding a course to the sorted list does not function (see below comment)
    class PeopleApp
    {
            ...
            while (sAction != "quit")
            {
                ...
                switch (sAction)
                {
                    ...
                    case "list":
                        ...
                        //lists every course that was entered by the user from courses.sortedList
                        foreach (KeyValuePair<string, Course> thisEntry in courses.sortedList)
                        {
                            
                            Course thisCourse = (Course)thisEntry.Value;

                            Console.Write($"{i + 1}: {thisCourse.courseCode} | {thisCourse.description} | {thisCourse.schedule}");
        ...
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                do
                {
                    ...
                    if (courseCode.Length > 0 && courseCode != " ")
                    {

                        //courses.sortedList.Add(courseCode,);
                        //Im not sure what the parameters are supposed to be here
                    }
                    else
                    {
                        break;
